  ...
  static StyledElement _processInlineWhitespaceRecursive(
    ...
    if (tree is TextContentElement) {
      ...
      if (textIndex < 1 &&
          ...
          (elementIndex < 1 || (elementIndex >= 1 && parentNodes?[elementIndex - 1] is html.Text && parentNodes![elementIndex - 1].text!.endsWith(" ")))) {
        tree.text = tree.text!.replaceFirst(' ', '');
      } else if (textIndex >= 1 && tree.text!.startsWith(' ') && tree.element?.nodes[textIndex - 1] is html.Element && (tree.element?.nodes[textIndex - 1] as html.Element).localName == "br") {
        tree.text = tree.text!.replaceFirst(' ', '');
      }
    ...
    tree.children.forEachIndexed((index, child) {
      if (child is EmptyContentElement) {
        toRemove.add(child);
          ...
          ((tree.name == "body" && (index == 0 || index + 1 == tree.children.length || tree.children[index - 1].style.display == Display.block || tree.children[index + 1].style.display == Display.block)) || tree.name == "ul") &&
      ...
      lastChildBlock = (child.style.display == Display.block || child.style.display == Display.listItem || (child is TextContentElement && child.text == '\n'));
    });
